table 51533351 "Perdiem Header"
{
    //DrillDownPageID = "Perdiem List";
    //LookupPageID = "Perdiem List";
    ...
    fields
    {
        ...
        {
            CalcFormula = Sum("Perdiem Lines".Amount WHERE(No = FIELD("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = Sum("Perdiem Lines"."Amount LCY" WHERE(No = FIELD("No.")));
            Editable = false;
            FieldClass = FlowField;
        }
        field(80; "Document Type"; Option)
        {
            OptionMembers = "Payment Voucher","Petty Cash"," ",Perdiem;
        }
        ...
        field(481; "Imprest Surrender No."; Code[20])
        {
            TableRelation = "Imprest Surrender Header".No;
        }
        field(482; "Staff Advance Surrender No."; Code[20])
        {
            TableRelation = "Staff Advance Surrender Header".No WHERE("Account No." = FIELD("Account No."));
        }
        field(50002; "Perdiem type"; Option)
        {
            OptionMembers = " ",Half,Quarter,Full;

            trigger OnValidate()
            begin
                if "Perdiem type" = "Perdiem type"::Half then Error('Half Perdiem Is UnAvailable');
            end;
        }
        field(50003; "Date of Departure"; Date)
        {
            ...
            begin
                ...
                if ImpLines.FindFirst then begin
                    ImpLines."No of Days" := ("Date of Return" - "Date of Departure");
                end;
            end;
        }
        field(50004; "Date of Return"; Date)
        {
            ...
            trigger OnValidate()
            begin
                if "Date of Return" < "Date of Departure" then
                    Error('Date of return' + ' ' + Format("Date of Return") + ' ' + 'cannot be earlier than the departure date' + ' ' + Format("Date of Departure"));
                ImpLines.Reset;
                ImpLines.SetRange(ImpLines.No, "No.");
                if ImpLines.FindFirst then begin
                    ImpLines."No of Days" := ("Date of Return" - "Date of Departure");
        ...
        field(50020; Changing; Boolean)
        {
        }
        field(50024; "Eft Batch"; Code[10])
        {
        }
        field(50027; "Incoming Document Entry No."; Integer)
        {
        }
        field(50028; RecID; RecordID)
        {
    ...
    begin
        ...
        if "No." = '' then begin
            GenLedgerSetup.Get();
            GenLedgerSetup.TestField(GenLedgerSetup."Perdiem Nos");
            NoSeriesMgt.InitSeries(GenLedgerSetup."Perdiem Nos", xRec."No. Series", 0D, "No.", "No. Series");
        end;
    ...
    var
        ...
        CashierLinks: Record "Receipts and Payment Types";
        ...
        PVSteps: Record "Purchase Header";
        RespCenter: Record "Responsibility Center";
        UserMgt: Codeunit "User Setup Management";
        Text001: Label 'Your identification is set up to process from %1 %2 only.';
        Pline: Record "Perdiem Lines";
        CurrExchRate: Record "Currency Exchange Rate";
        ImpLines: Record "Perdiem Lines";
    ...
    procedure Fn_CancelBudget()
    begin
        Commitments.Reset;
        Commitments.SetRange(Commitments."Document Type", Commitments."Document Type"::Perdiem);
