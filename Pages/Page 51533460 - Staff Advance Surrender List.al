page 51533460 "Staff Advance Surrender List"
{
            ...
            group(Functions)
            {
                ...
                action(Post)
                {
                    ...
                    begin
                        ...
                        PVHeader.Reset;
                        PVHeader.SetRange(PVHeader."Creation Doc No.","Imprest Issue Doc. No");
                ...
                action(Print)
                {
                    ...
                    trigger OnAction()
                    begin
                        if Status <> Status::Approved then Error('You can only print approved documents');
                        Reset;
                        SetFilter(No,No);
    ...
    begin
        ...
        SClaimHdr.Validate("Account No.","Account No.");
        //SClaimHdr.Payee := Payee;
        SClaimHdr."Responsibility Center" := "Responsibility Center";
        SClaimHdr.Validate("Global Dimension 1 Code","Global Dimension 1 Code");
        SClaimHdr.Validate("Shortcut Dimension 2 Code","Global Dimension 2 Code");
        SClaimHdr."Dimension Set ID" := "Dimension Set ID";
        SClaimHdr.Purpose := 'Advance '+No+' Over Expenditure Claim';
        SClaimHdr."Creation Doc No" := No;
        ...
        SClaimHdr.Insert();
          ...
          SClaimLines."Account No:":= "Account No.";
          SClaimLines."Account Name" := 'Advance '+No+' Over Expenditure Claim';
          SClaimLines.Amount := -Difference;
          ...
          SClaimLines.Purpose := 'Advance '+No+' Over Expenditure Claim';
          SClaimLines.Insert(true);

        Message('Staff Claim %1 automatically created and approved, Please post it to effect payment to the staff %2',SClaimHdr."No.",Payee);
    end;
    ...
    begin
         HasLines:=false;
         PayLines.Reset;
         PayLines.SetRange(PayLines."Surrender Doc No.",No);
          if PayLines.Find('-') then
        repeat
            PayLines.Rec.TestField(PayLines."Amount LCY");
             HasLines:=true;
             exit(HasLines);
        until PayLines.Next = 0;
    end;
